...
public class JDBC {
	...
	public static void findExistingLineUp() { // change name of func later,
		// example one
		try {
	...
	public static void findNearestPanel() { // change name of func later,
		// example one
		try {
			Statement stat = connection.createStatement();
	...
	public static void findNearestPort() { // change name of func later,
													// example one
		try {
			Statement stat = connection.createStatement();
			ResultSet result = stat.executeQuery("SELECT "
	...
	public static void getLineUpFrom_LineUpSYS_I() { // change name of func later,
													// example one
		try {
			Statement stat = connection.createStatement();
	...
	public static void getOpenPossibleConnections() { // change name of func later,
													// example one
		try {
			Statement stat = connection.createStatement();
			ResultSet result = stat.executeQuery("SELECT *"
	...
	public static void getPipes() { // change name of func later,
													// example one
		try {
			Statement stat = connection.createStatement();
	...
	public static void graphInformation() { // change name of func later,
													// example one
		try {
			Statement stat = connection.createStatement();
			...
			while (result.next()) {
				float cp1_id = result.getFloat("CP1_ID");
				String cp1_cn = result.getString("CP1_CN");
				float cp2_id = result.getFloat("CP2_ID");
				String cp2_cn = result.getString("CP2_CN");
				float pipelength = result.getFloat("PipeLength");
				System.out.println(cp1_id + "\t" + cp1_cn + "\t" + cp2_id + "\t" + cp2_cn + "\t" + pipelength);
			}
		}
